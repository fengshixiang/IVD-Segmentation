...
def SegSE_layer(x, out_dim, ratio, rate, name):
    with tf.name_scope("SegSE_{}".format(name)):
        stddev = np.sqrt(2 / (3 ** 2 * out_dim))
        w1 = weight_variable([3, 3, out_dim, out_dim], stddev, name="w1")
        b1 = bias_variable([out_dim], name="b1")
        squeeze = tf.nn.bias_add(tf.nn.atrous_conv2d(x, w1, rate=4, padding="SAME"), b1)
        squeeze = tf.nn.relu(squeeze)
        w2 = weight_variable([1, 1, out_dim, out_dim], stddev)
        b2 = bias_variable([out_dim], name="b2")
        squeeze = tf.nn.bias_add(tf.nn.conv2d(squeeze, w2, strides=[1, 1, 1, 1], padding="SAME"), b2)
        excitation = tf.nn.sigmoid(squeeze)
...
def inception_conv_SE(x, in_dim, out_dim, ratio, keep_prob_, training):
    with tf.name_scope("conv2d"):
        stddev = np.sqrt(2 / (3 ** 2 * out_dim))
        ...
        # SE
        conv_input = tf.concat([conv2d_1, conv2d_2, conv2d_3, conv2d_4, conv2d_5], axis=3)
        squeeze = tf.reduce_mean(conv_input, axis=[1,2])
        excitation = tf.layers.dense(inputs=squeeze, use_bias=False, units=int(out_dim*5//ratio), name="scSE_{}_1".format(name))
        excitation = tf.nn.relu(excitation)
        excitation = tf.layers.dense(inputs=excitation, use_bias=False, units=out_dim*5, name="scSE_{}_2".format(name))
        excitation = tf.nn.sigmoid(excitation)
        excitation = tf.reshape(excitation, [-1,1,1,out_dim*5])
        conv2d_input = x * excitation
...
def inception_conv_my_1(x, in_dim, out_dim, keep_prob_, training):
    with tf.name_scope("conv2d"):
        stddev = np.sqrt(2 / (3 ** 2 * out_dim))

        w1 = weight_variable([1, 1, in_dim, out_dim], stddev, name="w1")
        conv2d_1 = tf.nn.conv2d(x, w1, strides=[1, 1, 1, 1], padding="SAME")
        b1 = bias_variable([out_dim], name="b1")
        conv2d_1 = tf.nn.bias_add(conv2d_1, b1)
        #conv2d_1 = tf.layers.batch_normalization(conv2d_1, training=training, name=w1.name[:-2]+'/bn')
        conv2d_1 = tf.nn.relu(conv2d_1)

        w2 = weight_variable([3, 3, in_dim, out_dim], stddev, name="w2")
        conv2d_2 = tf.nn.conv2d(x, w2, strides=[1, 1, 1, 1], padding="SAME")
        b2 = bias_variable([out_dim], name="b2")
        conv2d_2 = tf.nn.bias_add(conv2d_2, b2)
        #conv2d_2 = tf.layers.batch_normalization(conv2d_2, training=training, name=w2.name[:-2]+'/bn')
        conv2d_2 = tf.nn.relu(conv2d_2)

        w3 = weight_variable([5, 5, in_dim, out_dim], stddev, name="w3")
        conv2d_3 = tf.nn.conv2d(x, w3, strides=[1, 1, 1, 1], padding="SAME")
        b3 = bias_variable([out_dim], name="b3")
        conv2d_3 = tf.nn.bias_add(conv2d_3, b3)
        #conv2d_3 = tf.layers.batch_normalization(conv2d_3, training=training, name=w3.name[:-2]+'/bn')
        conv2d_3 = tf.nn.relu(conv2d_3)

        w4 = weight_variable([3, 3, in_dim, out_dim], stddev, name="w4")
        conv2d_4 = tf.nn.atrous_conv2d(x, w4, rate=2, padding="SAME")
        b4 = bias_variable([out_dim], name="b4")
        conv2d_4 = tf.nn.bias_add(conv2d_4, b4)
        #conv2d_4 = tf.layers.batch_normalization(conv2d_4, training=training, name=w4.name[:-2]+'/bn')
        conv2d_4 = tf.nn.relu(conv2d_4)

        w5 = weight_variable([3, 3, in_dim, out_dim], stddev, name="w5")
        conv2d_5 = tf.nn.atrous_conv2d(x, w5, rate=4, padding="SAME")
        b5 = bias_variable([out_dim], name="b5")
        conv2d_5 = tf.nn.bias_add(conv2d_5, b5)
        #conv2d_5 = tf.layers.batch_normalization(conv2d_5, training=training, name=w5.name[:-2]+'/bn')
        conv2d_5 = tf.nn.relu(conv2d_5)
        ...
        w0 = weight_variable([1, 1, in_dim, out_dim], stddev, name="w0")
        conv2d_0 = tf.nn.conv2d(x, w0, strides=[1, 1, 1, 1], padding="SAME")
        conv2d_0 = tf.layers.batch_normalization(conv2d_0, training=training, name=w0.name[:-2]+'/bn')
        conv2d_0 = tf.nn.relu(conv2d_0)
...
def inception_conv_my_2(x, in_dim, out_dim, keep_prob_, training):
    with tf.name_scope("conv2d"):
        stddev = np.sqrt(2 / (3 ** 2 * out_dim))
...
def inception_conv_deep(x, in_dim, out_dim, keep_prob_, training):
    with tf.name_scope("conv2d"):
        stddev = np.sqrt(2 / (3 ** 2 * out_dim))
        ...
        conv2d_input = tf.concat([conv2d_1, conv2d_2, conv2d_3], axis=3)
        w6 = weight_variable([1, 1, out_dim*3, out_dim], stddev, name="w6")
        conv2d_6 = tf.nn.conv2d(conv2d_input, w6, strides=[1, 1, 1, 1], padding="SAME")
        conv2d_6 = tf.layers.batch_normalization(conv2d_6, training=training, name=w6.name[:-2] + '/bn')
        conv2d_6 = tf.nn.relu(conv2d_6)
