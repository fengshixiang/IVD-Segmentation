...
from layers import (weight_variable, weight_variable_devonc, bias_variable,
                    conv2d, conv2d_2, deconv2d, deconv2d_2, max_pool, max_pool_xz, pixel_wise_softmax, cross_entropy,
                    inception_conv, inception_conv_asym, dense_link, cropCenter, deconv2d_xz, deconv2d_xz_2)
...
def create_IVD_net_asym(x, keep_prob, channels, n_class, layers=5, features_root=32, summaries=True, training=True):
    logging.info(
        "Layers {layers}, features {features}".format(
            layers=layers,
            features=features_root))
    ...
    for layer in range(0, layers):
        with tf.name_scope("down_conv_{}".format(str(layer))):
            ...
            if layer==0:
                with tf.name_scope("fat"):
                    in_node = tf.reshape(x_image[..., 0],[batch_size, nx, ny, 1])
                    conv = inception_conv_asym(in_node, 1, features, keep_prob, training)
                    fat_dw_h_convs[layer] = conv
                    fat_pools[layer] = max_pool(conv, 2)
                with tf.name_scope("inn"):
                    in_node = tf.reshape(x_image[..., 1],[batch_size, nx, ny, 1])
                ...
                with tf.name_scope("fat"):
                    in_node = tf.concat([fat_pools[0], inn_pools[0], wat_pools[0], opp_pools[0]], 3)
                    fat_inputs[layer] = in_node
                    conv = inception_conv_asym(in_node, features_root*4, features, keep_prob, training)
                    fat_dw_h_convs[layer] = conv
                    fat_pools[layer] = max_pool(conv, 2)
                with tf.name_scope("inn"):
                    in_node = tf.concat([inn_pools[0], wat_pools[0], opp_pools[0], fat_pools[0]], 3)
                ...
                with tf.name_scope("fat"):
                    ...
                    in_node = tf.concat([in_node, tmp_node], 3)
                    fat_inputs[layer] = in_node
                    conv = inception_conv_asym(in_node, features_root*12, features, keep_prob, training)
                    fat_dw_h_convs[layer] = conv
                    fat_pools[layer] = max_pool(conv, 2)
                with tf.name_scope("inn"):
                    in_node = tf.concat([inn_pools[1], wat_pools[1], opp_pools[1], fat_pools[1]], 3)
                ...
                with tf.name_scope("fat"):
                    ...
                    in_node = tf.concat([in_node, tmp_node], 3)
                    fat_inputs[layer] = in_node
                    conv = inception_conv_asym(in_node, features_root*28, features, keep_prob, training)
                    fat_dw_h_convs[layer] = conv
                    fat_pools[layer] = max_pool(conv, 2)
                with tf.name_scope("inn"):
                    in_node = tf.concat([inn_pools[2], wat_pools[2], opp_pools[2], fat_pools[2]], 3)
                ...
                tmp_node = cropCenter(fat_inputs[3], in_node)
                in_node = tf.concat([in_node, tmp_node], 3)
                conv = inception_conv_asym(in_node, features_root*60, features, keep_prob, training)
                up_h_convs[layer] = conv
        ...
        with tf.name_scope("down_conv_{}".format(str(layer))):
            ...
            if layer==0:
                with tf.name_scope("fat"):
                    ...
                    fat_dw_h_convs[layer] = conv
                    fat_pools[layer] = max_pool(conv, 2)
                    #fat_pools[layer] = max_pool(conv, 2, features)
                with tf.name_scope("inn"):
                    in_node = tf.reshape(x_image[..., 1],[batch_size, nx, ny, 1])
                    conv = inception_conv(in_node, 1, features, keep_prob, training)
                    inn_dw_h_convs[layer] = conv
                    inn_pools[layer] = max_pool(conv, 2)
                    #inn_pools[layer] = max_pool(conv, 2, features)
                with tf.name_scope("wat"):
                    in_node = tf.reshape(x_image[..., 2],[batch_size, nx, ny, 1])
                    conv = inception_conv(in_node, 1, features, keep_prob, training)
                    wat_dw_h_convs[layer] = conv
                    wat_pools[layer] = max_pool(conv, 2)
                    #wat_pools[layer] = max_pool(conv, 2, features)
                with tf.name_scope("opp"):
                    in_node = tf.reshape(x_image[..., 3],[batch_size, nx, ny, 1])
                    conv = inception_conv(in_node, 1, features, keep_prob, training)
                    opp_dw_h_convs[layer] = conv
                    opp_pools[layer] = max_pool(conv, 2)
                    #opp_pools[layer] = max_pool(conv, 2, features)
            elif layer==1:
                with tf.name_scope("fat"):
                    in_node = tf.concat([fat_pools[0], inn_pools[0], wat_pools[0], opp_pools[0]], 3)
                    fat_inputs[layer] = in_node
...
def create_edge_net_5(x, keep_prob, channels, n_class, layers=5, features_root=32, summaries=True, training=True):
    # reduce link
    logging.info(
        "Layers {layers}, features {features}".format(
            layers=layers,
        ...
        with tf.name_scope("down_conv_{}".format(str(layer))):
            ...
            if layer==0:
                ...
                with tf.name_scope("fat"):
                    ...
                    in_node = tf.concat([in_node, tmp_node], 3)
                    fat_inputs[layer] = in_node
                    conv = inception_conv(in_node, features_root*3, features, keep_prob, training)
                    fat_dw_h_convs[layer] = conv
                    fat_pools[layer] = max_pool(conv, 2)
                    #fat_pools[layer] = max_pool(conv, 2, features)
                with tf.name_scope("inn"):
                ...
                with tf.name_scope("fat"):
                    in_node = tf.concat([fat_pools[2], inn_pools[2], wat_pools[2], opp_pools[2]], 3)
                    tmp_node_1 = tf.concat([fat_pools[1], inn_pools[1], wat_pools[1], opp_pools[1]], 3)
                    tmp_node_1 = cropCenter(tmp_node_1, in_node)
                    tmp_node_2 = tf.concat([fat_pools[0], inn_pools[0], wat_pools[0], opp_pools[0]], 3)
                    tmp_node_2 = cropCenter(tmp_node_2, in_node)
                    in_node = tf.concat([in_node, tmp_node_1, tmp_node_2], 3)
                    fat_inputs[layer] = in_node
                    conv = inception_conv(in_node, features_root*28, features, keep_prob, training)
                    fat_dw_h_convs[layer] = conv
                    fat_pools[layer] = max_pool(conv, 2)
                ...
                with tf.name_scope("inn"):
                    in_node = tf.concat([inn_pools[2], wat_pools[2], opp_pools[2], fat_pools[2]], 3)
                    tmp_node_1 = tf.concat([inn_pools[1], wat_pools[1], opp_pools[1], fat_pools[1]], 3)
                    tmp_node_1 = cropCenter(tmp_node_1, in_node)
                    tmp_node_2 = tf.concat([inn_pools[0], wat_pools[0], opp_pools[0], fat_pools[0]], 3)
                    tmp_node_2 = cropCenter(tmp_node_2, in_node)
                    in_node = tf.concat([in_node, tmp_node_1, tmp_node_2], 3)
                    #inn_inputs[layer] = in_node
                    conv = inception_conv(in_node, features_root*28, features, keep_prob, training)
                ...
                with tf.name_scope("wat"):
                    in_node = tf.concat([wat_pools[2], opp_pools[2], fat_pools[2], inn_pools[2]], 3)
                    tmp_node_1 = tf.concat([wat_pools[1], opp_pools[1], fat_pools[1], inn_pools[1]], 3)
                    tmp_node_1 = cropCenter(tmp_node_1, in_node)
                    tmp_node_2 = tf.concat([wat_pools[0], opp_pools[0], fat_pools[0], inn_pools[0]], 3)
                    tmp_node_2 = cropCenter(tmp_node_2, in_node)
                    in_node = tf.concat([in_node, tmp_node_1, tmp_node_2], 3)
                    #wat_inputs[layer] = in_node
                    conv = inception_conv(in_node, features_root*28, features, keep_prob, training)
                ...
                with tf.name_scope("opp"):
                    in_node = tf.concat([opp_pools[2], fat_pools[2], inn_pools[2], wat_pools[2]], 3)
                    tmp_node_1 = tf.concat([opp_pools[1], fat_pools[1], inn_pools[1], wat_pools[1]], 3)
                    tmp_node_1 = cropCenter(tmp_node_1, in_node)
                    tmp_node_2 = tf.concat([opp_pools[0], fat_pools[0], inn_pools[0], wat_pools[0]], 3)
                    tmp_node_2 = cropCenter(tmp_node_2, in_node)
                    in_node = tf.concat([in_node, tmp_node_1, tmp_node_2], 3)
                    #opp_inputs[layer] = in_node
                    conv = inception_conv(in_node, features_root*28, features, keep_prob, training)
...
def create_edge_net_6(x, keep_prob, channels, n_class, layers=5, features_root=32, summaries=True, training=True):
    # reduce link
    logging.info(
        "Layers {layers}, features {features}".format(
            layers=layers,
    ...
    for layer in range(0, layers):
        with tf.name_scope("down_conv_{}".format(str(layer))):
            ...
            if layer==0:
                ...
                with tf.name_scope("fat"):
                    in_node = tf.concat([fat_pools[1], inn_pools[1], wat_pools[1], opp_pools[1]], 3)
                    tmp_node = tf.concat([fat_pools[0], inn_pools[0], wat_pools[0], opp_pools[0]], 3)
                    tmp_node = cropCenter(tmp_node, in_node)
                    in_node = tf.concat([in_node, tmp_node], 3)
                    fat_inputs[layer] = in_node
                    conv = inception_conv(in_node, features_root*12, features, keep_prob, training)
                    fat_dw_h_convs[layer] = conv
                ...
                with tf.name_scope("inn"):
                    in_node = tf.concat([inn_pools[1], wat_pools[1], opp_pools[1], fat_pools[1]], 3)
                    tmp_node = tf.concat([inn_pools[0], wat_pools[0], opp_pools[0], fat_pools[0]], 3)
                    tmp_node = cropCenter(tmp_node, in_node)
                    in_node = tf.concat([in_node, tmp_node], 3)
                    inn_inputs[layer] = in_node
                    conv = inception_conv(in_node, features_root*12, features, keep_prob, training)
                ...
                with tf.name_scope("wat"):
                    in_node = tf.concat([wat_pools[1], opp_pools[1], fat_pools[1], inn_pools[1]], 3)
                    tmp_node = tf.concat([wat_pools[0], opp_pools[0], fat_pools[0], inn_pools[0]], 3)
                    tmp_node = cropCenter(tmp_node, in_node)
                    in_node = tf.concat([in_node, tmp_node], 3)
                    wat_inputs[layer] = in_node
                    conv = inception_conv(in_node, features_root*12, features, keep_prob, training)
                ...
                with tf.name_scope("opp"):
                    in_node = tf.concat([opp_pools[1], fat_pools[1], inn_pools[1], wat_pools[1]], 3)
                    tmp_node = tf.concat([opp_pools[0], fat_pools[0], inn_pools[0], wat_pools[0]], 3)
                    tmp_node = cropCenter(tmp_node, in_node)
                    in_node = tf.concat([in_node, tmp_node], 3)
                    opp_inputs[layer] = in_node
                    conv = inception_conv(in_node, features_root*12, features, keep_prob, training)
...
def create_edge_net_6_4layer(x, keep_prob, channels, n_class, layers=4, features_root=32, summaries=True, training=True):
    # reduce link
    logging.info(
        "Layers {layers}, features {features}".format(
            layers=layers,
...
def create_edge_net_6_3layer(x, keep_prob, channels, n_class, layers=4, features_root=32, summaries=True, training=True):
    # reduce link
    logging.info(
        "Layers {layers}, features {features}".format(
            layers=layers,
...
def create_edge_net_7(x, keep_prob, channels, n_class, layers=5, features_root=32, summaries=True, training=True):
    # using crop and average-pooling for different links
    logging.info(
        "Layers {layers}, features {features}".format(
            layers=layers,
            features=features_root))
        ...
        with tf.name_scope("down_conv_{}".format(str(layer))):
            ...
            if layer==0:
                ...
                with tf.name_scope("fat"):
                    ...
                    tmp_node_1 = cropCenter(tmp_node_1, in_node)
                    tmp_node_2 = tf.concat([fat_pools[0], inn_pools[0], wat_pools[0], opp_pools[0]], 3)
                    tmp_node_2 = tf.nn.avg_pool(tmp_node_2, ksize=[1, 4, 4, 1], strides=[1, 4, 4, 1], padding='VALID')
                    in_node = tf.concat([in_node, tmp_node_1, tmp_node_2], 3)
                    fat_inputs[layer] = in_node
                    conv = inception_conv(in_node, features_root*28, features, keep_prob, training)
                    fat_dw_h_convs[layer] = conv
                ...
                in_node = tf.concat([fat_pools[3], inn_pools[3], wat_pools[3], opp_pools[3]], 3)
                tmp_node_1 = tf.concat([fat_pools[2], inn_pools[2], wat_pools[2], opp_pools[2]], 3)
                tmp_node_1 = cropCenter(tmp_node_1, in_node)
                tmp_node_2 = tf.concat([fat_pools[1], inn_pools[1], wat_pools[1], opp_pools[1]], 3)
                tmp_node_2 = tf.nn.avg_pool(tmp_node_2, ksize=[1, 4, 4, 1], strides=[1, 4, 4, 1], padding='VALID')
                tmp_node_3 = tf.concat([fat_pools[0], inn_pools[0], wat_pools[0], opp_pools[0]], 3)
                tmp_node_3 = tf.nn.avg_pool(tmp_node_3, ksize=[1, 8, 8, 1], strides=[1, 8, 8, 1], padding='VALID')
                in_node = tf.concat([in_node, tmp_node_1, tmp_node_2, tmp_node_3], 3)
                conv = inception_conv(in_node, features_root*60, features, keep_prob, training)
                up_h_convs[layer] = conv
...
def create_edge_net_axis_xz_yz(x, keep_prob, channels, n_class, layers=5, features_root=32, summaries=True, training=True):
    # reduce link
    logging.info(
        "Layers {layers}, features {features}".format(
            layers=layers,
        ...
        with tf.name_scope("down_conv_{}".format(str(layer))):
            ...
            if layer==0:
                ...
                with tf.name_scope("inn"):
                    ...
                    conv = inception_conv(in_node, 1, features, keep_prob, training)
                    inn_dw_h_convs[layer] = conv
                    inn_pools[layer] = max_pool_xz(conv, 1, 2)
                with tf.name_scope("wat"):
                    in_node = tf.reshape(x_image[..., 2],[batch_size, nx, ny, 1])
                    conv = inception_conv(in_node, 1, features, keep_prob, training)
                    wat_dw_h_convs[layer] = conv
                    wat_pools[layer] = max_pool_xz(conv, 1, 2)
                with tf.name_scope("opp"):
                    in_node = tf.reshape(x_image[..., 3],[batch_size, nx, ny, 1])
                    conv = inception_conv(in_node, 1, features, keep_prob, training)
                    opp_dw_h_convs[layer] = conv
                    opp_pools[layer] = max_pool_xz(conv, 1, 2)
            elif layer==1:
                with tf.name_scope("fat"):
                    in_node = fat_pools[0]
                    fat_inputs[layer] = in_node
                ...
                with tf.name_scope("inn"):
                    ...
                    conv = inception_conv(in_node, features_root, features, keep_prob, training)
                    inn_dw_h_convs[layer] = conv
                    inn_pools[layer] = max_pool_xz(conv, 2, 2)
                with tf.name_scope("wat"):
                    in_node = wat_pools[0]
                    wat_inputs[layer] = in_node
                    conv = inception_conv(in_node, features_root, features, keep_prob, training)
                    wat_dw_h_convs[layer] = conv
                    wat_pools[layer] = max_pool_xz(conv, 2, 2)
                with tf.name_scope("opp"):
                    in_node = opp_pools[0]
                    opp_inputs[layer] = in_node
                    conv = inception_conv(in_node, features_root, features, keep_prob, training)
                    opp_dw_h_convs[layer] = conv
                    opp_pools[layer] = max_pool_xz(conv, 2, 2)
            elif layer==2:
                with tf.name_scope("fat"):
                    in_node = tf.concat([fat_pools[1], inn_pools[1], wat_pools[1], opp_pools[1]], 3)
                    tmp_node = tf.concat([fat_pools[0], inn_pools[0], wat_pools[0], opp_pools[0]], 3)
    ...
    for layer in range(1, 0, -1):
        with tf.name_scope("output_{}".format(str(layer))):
            in_node = up_h_convs[layer]
            conv = conv2d_2(in_node, features_root*2**layer, 2, keep_prob)
            deconv = deconv2d_xz_2(conv, 2, 2, 1, 2**layer, training)
            up_h_convs["out_{}".format(layer)] = deconv
    in_node = tf.concat([up_h_convs["out"], up_h_convs["out_1"]], 3)
    w_out = weight_variable([1, 1, 4, 2], stddev, name="w_out")
    b_out = bias_variable([2], name="b_out")
...
def create_edge_net_axis_xz_yz_4layer(x, keep_prob, channels, n_class, layers=4, features_root=32, summaries=True, training=True):
    logging.info(
        "Layers {layers}, features {features}".format(
            layers=layers,
            features=features_root))
...
def create_edge_net_axis_xz_yz_3layer(x, keep_prob, channels, n_class, layers=4, features_root=32, summaries=True, training=True):
    # reduce link
    logging.info(
        "Layers {layers}, features {features}".format(
            layers=layers,
...
class Unet(object):
    ...
    def __init__(self, channels=3, n_class=2, cost="cross_entropy", cost_kwargs={}, **kwargs):
        ...
        logits, self.variables= create_edge_net_axis_xz_yz_3layer(self.x, self.keep_prob, channels, n_class, **kwargs)
    ...
    def _get_cost(self, logits, cost_name, cost_kwargs):
        ...
        with tf.name_scope("cost"):
            ...
            if cost_name == "cross_entropy":
                ...
                for logit in logits:
                    flat_logit = tf.reshape(logit, [-1, self.n_class])
                    flat_logit = tf.nn.softmax(flat_logit)
                    flat_label = tf.reshape(self.y, [-1, self.n_class])
                    loss -= tf.reduce_mean(flat_label*(1-flat_logit)**2*tf.log(tf.clip_by_value(flat_logit,1e-10,1.0)), name="cross_entropy")
                ...
                for logit in logits:
                    ...
                    edge_pre = tf.image.sobel_edges(logit / tf.reduce_max(logits))
                ...
                for logit in logits:
                    ...
                    loss += tf.sqrt(tf.nn.l2_loss(edge_gt - edge_pre)*2)/256
            elif cost_name == "EPE_4":
                loss = 0
                for logit in logits:
                    edge_gt = tf.image.sobel_edges(self.y / tf.reduce_max(self.y))
                    edge_gt = tf.sqrt(edge_gt[...,0]**2 + edge_gt[...,1]**2 + 0.00001)
                    edge_gt /= tf.reduce_max(edge_gt)
                    edge_pre = tf.image.sobel_edges(logit / tf.reduce_max(logit))
                    edge_pre = tf.sqrt(edge_pre[..., 0]**2 + edge_pre[..., 1]**2 + 0.00001)
                    edge_pre /= tf.reduce_max(edge_pre)
                    loss += tf.sqrt(tf.nn.l2_loss(edge_gt - edge_pre)*2)
