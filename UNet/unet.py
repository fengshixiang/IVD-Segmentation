...
def create_conv_net_2(x, keep_prob, channels, n_class, layers=5, features_root=32, summaries=True, training=True):
    # replace concat with add when upsample
    logging.info(
        "Layers {layers}, features {features}".format(
            layers=layers,
            features=features_root))
    ...
    for layer in range(layers - 2, -1, -1):
        with tf.name_scope("up_conv_{}".format(str(layer))):
            features = 2 ** (layer + 1) * features_root
            h_deconv = tf.nn.relu(deconv2d(in_node, features, features//2, training))
            h_deconv_concat = (dw_h_convs[layer] + h_deconv) / 2
            deconv[layer] = h_deconv_concat

            conv1 = conv2d(h_deconv_concat, features//2, features//2, keep_prob, training)
            conv2 = conv2d(conv1, features//2, features//2, keep_prob, training)
            in_node = conv2
            up_h_convs[layer] = in_node
...
def create_UNet_SE(x, keep_prob, channels, n_class, layers=5, features_root=32, summaries=True, training=True):
    logging.info(
        "Layers {layers}, features {features}".format(
            layers=layers,
            features=features_root))
...
class Unet(object):
    ...
    def _get_cost(self, logits, cost_name, cost_kwargs):
        ...
        with tf.name_scope("cost"):
            ...
            if cost_name == 'CE':
            ...
            elif cost_name == "CE+edge_distillation":
                loss = 0
                ...
                loss += 0.01*tf.reduce_mean(-tf.nn.softmax_cross_entropy_with_logits_v2(logits=flat_logit_1[0], labels=flat_logit_1[0]/(flat_logit_2[0] + 0.000001)))
                ...
                flat_logit_1 = tf.reshape(logit_1, [-1, self.n_class])
                flat_logit_2 = tf.reshape(logit_2, [-1, self.n_class])
                loss += 0.01*tf.reduce_mean(-tf.nn.softmax_cross_entropy_with_logits_v2(logits=flat_logit_2[0], labels=flat_logit_2[0]/(flat_logit_1[0] + 0.000001)))
