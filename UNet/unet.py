        ...
        with tf.name_scope("down_conv_{}".format(str(layer))):
            ...
            if layer==0:
                ...
                with tf.name_scope("fat"):
                    ...
                    tmp_node_1 = cropCenter(tmp_node_1, in_node)
                    tmp_node_2 = tf.concat([fat_pools[0], inn_pools[0], wat_pools[0], opp_pools[0]], 3)
                    tmp_node_2 = tf.nn.avg_pool(tmp_node_2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID')
                    in_node = tf.concat([in_node, tmp_node_1, tmp_node_2], 3)
                    fat_inputs[layer] = in_node
                    conv = inception_conv(in_node, features_root*28, features, keep_prob, training)
                    fat_dw_h_convs[layer] = conv
                ...
                tmp_node_3 = tf.nn.avg_pool(tmp_node_3, ksize=[1, 4, 4, 1], strides=[1, 4, 4, 1], padding='VALID')
        ...
        with tf.name_scope("cost"):
            ...
            if cost_name == "cross_entropy":
                ...
                for logit in logits:
                    flat_logit = tf.reshape(logit, [-1, self.n_class])
                    flat_label = tf.reshape(self.y, [-1, self.n_class])
                    loss_tmp = tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(logits=flat_logit, targets=flat_label, pos_weight=0.005))
                ...
                for logit in logits:
                    ...
                    edge_pre = tf.image.sobel_edges(logit / tf.reduce_max(logit))
                    edge_pre = tf.abs(edge_pre[...,0]) + tf.abs(edge_pre[...,1])
                    loss_tmp += tf.sqrt(tf.nn.l2_loss(edge_gt - edge_pre)*2)/256/20
                    loss += loss_tmp
