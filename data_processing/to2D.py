...
npy_data_address = '/DATA5_DB8/data/sxfeng/data/IVDM3Seg/npy_data_label_img_8m'
data_save_address = '/DATA5_DB8/data/sxfeng/data/IVDM3Seg/2D_data/img_label_8m' 
start_index = 0
end_index = 36
...
for addr in index_list:
	...
	for index in range(start_index, end_index):
		'''
		if (index >=1 and index <=5) or (index >=29 and index <= 34):
			...
			for times in range(5):
				index_addr = os.path.join(patient_save_addr, '{}_{}'.format(index, times)) # 2D_data_slidesweight/01/0_0
		...
		if False:
			...
			plt.imshow(inn_image_arr, cmap='gray')
			plt.savefig(inn_image_save_addr)
			plt.imshow(image_label_arr, cmap='gray')
			plt.savefig(label_save_addr)
...
for addr in index_list:
	...
	opp_voxel_name = addr + '_opp.npy'
	wat_voxel_name = addr + '_wat.npy'
	fin_voxel_name = addr + '_fin.npy'
	iop_voxel_name = addr + '_iop.npy'
	win_voxel_name = addr + '_win.npy'
	wop_voxel_name = addr + '_wop.npy'
	label_name = addr + '_Labels.npy'
	...
	opp_voxel_addr = os.path.join(patient_addr, opp_voxel_name)
	wat_voxel_addr = os.path.join(patient_addr, wat_voxel_name)
	fin_voxel_addr = os.path.join(patient_addr, fin_voxel_name)
	iop_voxel_addr = os.path.join(patient_addr, iop_voxel_name)
	win_voxel_addr = os.path.join(patient_addr, win_voxel_name)
	wop_voxel_addr = os.path.join(patient_addr, wop_voxel_name)
	voxel_label_addr = os.path.join(patient_addr, label_name)
	...
	opp_voxel_arr = np.load(opp_voxel_addr)
	wat_voxel_arr = np.load(wat_voxel_addr)
	fin_voxel_arr = np.load(fin_voxel_addr)
	iop_voxel_arr = np.load(iop_voxel_addr)
	win_voxel_arr = np.load(win_voxel_addr)
	wop_voxel_arr = np.load(wop_voxel_addr)
	voxel_label_arr = np.load(voxel_label_addr)
	...
	for index in range(0, 256):
		...
		opp_image_arr = opp_voxel_arr[:, index]
		wat_image_arr = wat_voxel_arr[:, index]
		fin_image_arr = fin_voxel_arr[:, index]
		iop_image_arr = iop_voxel_arr[:, index]
		win_image_arr = win_voxel_arr[:, index]
		wop_image_arr = wop_voxel_arr[:, index]
		image_label_arr = voxel_label_arr[:, index]
		...
		opp_imagearr_save_addr = os.path.join(index_addr, '{}_{}_opp.npy'.format(addr, index))
		wat_imagearr_save_addr = os.path.join(index_addr, '{}_{}_wat.npy'.format(addr, index))
		fin_imagearr_save_addr = os.path.join(index_addr, '{}_{}_fin.npy'.format(addr, index))
		iop_imagearr_save_addr = os.path.join(index_addr, '{}_{}_iop.npy'.format(addr, index))
		win_imagearr_save_addr = os.path.join(index_addr, '{}_{}_win.npy'.format(addr, index))
		wop_imagearr_save_addr = os.path.join(index_addr, '{}_{}_wop.npy'.format(addr, index))
		labelarr_save_addr = os.path.join(index_addr, '{}_{}_label.npy'.format(addr, index))
		...
		np.save(opp_imagearr_save_addr, opp_image_arr)
		np.save(wat_imagearr_save_addr, wat_image_arr)
		np.save(fin_imagearr_save_addr, fin_image_arr)
		np.save(iop_imagearr_save_addr, iop_image_arr)
		np.save(win_imagearr_save_addr, win_image_arr)
		np.save(wop_imagearr_save_addr, wop_image_arr)
		np.save(labelarr_save_addr, image_label_arr)
"""
